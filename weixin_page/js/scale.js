(function(){
  ...
  var change_end_wraper=function(dy,cur,act){
    ...
    if (Math.abs(dy)>0){
      ...
      $act.animate({'-webkit-transform':'scale(1)'},300,'linear');
      ischange=true;
    }else{
      $cur.animate({'-webkit-transform':'scale(1)'},300,'linear');
    }
  ...
  var change_dy_wraper=function(dy,cur,act){
    ...
    curscale=1-Math.abs(dy/one);
    curscale=curscale<0.5?0.5:curscale;
    $cur.css({'-webkit-transform':'scale('+curscale+')'});
    actscale=0.5+Math.abs(dy/one);
    actscale=actscale>1?1:actscale;
    $act.css({'-webkit-transform':'scale('+actscale+')'});
    if (dy>0){
      $act.css({'-webkit-transform-origin':'top'});
      $cur.css({'-webkit-transform-origin':'bottom'});
    }else{
      $cur.css({'-webkit-transform-origin':'top'});
      $act.css({'-webkit-transform-origin':'bottom'});
    }
  };
